describe('Test property list block', () => {
    ...
    const interceptGetPropertiesRequest = () => {
        cy.intercept(
            {
                method: 'GET',
    ...
    before(() => {
        ...
        cy.get('.editor-post-title__input').type('Property List');
        cy.get('.edit-post-header-toolbar__inserter-toggle').click();
        cy.get('.editor-block-list-item-side-property-list').click({force: true});
        cy.saveCurrentPost();

        cy
            .url()
            .then(url => {
                interceptGetPropertiesRequest();

                const postId = url.match(/post=(\d+)/)[1];

                cy.visit(`/?page_id=${postId}`);
                cy.wait('@getProperties');
            });
    });

    it('Shows expected data points for each listing', () => {
      cy.get('.property-image');
      cy.get('.property-meta');
      cy.get('.property-price');
      cy.get('.property-address');
      cy.get('.property-list-date');
    });

    it('Changes button state when listing is favorited', () => {
        cy.get('.property:first-child .favorite-button').click();
        cy
            .get('.property:first-child .favorite-button img')
            .invoke('attr', 'src')
            .should('include', 'heart-fill.svg');
